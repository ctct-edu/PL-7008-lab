...
> **Copilot 生成前の確認事項**
>
> - Copilot の生成結果は実行ごとに異なり、ノードが重複したり、選択したノードが更新されずに新しいノードが末尾に追加されたりする場合があります。
> - **対策1:** 「Copilotで編集する」を実行する前に必ず**保存**しておきます。意図と異なる結果になった場合は、**元に戻す**アイコンから「1つ前の操作に戻る」または「最後に保存した状態に戻す」で復旧できます。
> - **対策2:** 既存ノードを変更したい場合は、対象ノードを選択した状態でプロンプトに「選択中のノードを」と明示してください（対応できるのはメッセージ・質問・条件ノードのみ）。
> - **対策3:** 生成後はキャンバス全体を確認し、意図しない重複・余分なノードがあれば省略記号（...）メニューから削除してください。

この演習では、エージェントにトピックを作成し、トリガーフレーズを追加します。

### タスク2.1 – Copilotを使用してトピックを追加する

1. **+ トピックを追加**を選択し、**Copilot で説明をもとに追加する**を選びます。新しいウィンドウが表示されます。

    ![トピック追加メニュー展開のスクリーンショット](../media/Lab03_011.JPG)

1. **トピックに名前を付ける** テキストボックスに次のテキストを入力します:

    ```
    お客様詳細
    ```

1. **次のトピックを作成...** テキストボックスに次のテキストを入力します:

    ```
    顧客の名前とメールアドレスを尋ねる
    ```

    ![Copilotトピック作成ダイアログのスクリーンショット](../media/Lab03_012.JPG)
...
1. 2番目の**質問**ノード**メールアドレスを教えてください**を選択します。
    ...
    ![オーサリングキャンバス（Copilotで編集するパネル）のスクリーンショット](../media/Lab03_013.JPG)

1. **Copilotで編集する**パネルの **何の操作を実行しますか?** フィールドに、以下のテキストを入力します:

    ```
    選択中の質問ノードのメッセージテキストを変更してください。現在の「メールアドレスを教えてください。」というテキストの前に、入力された名前に感謝の言葉を述べてから、メールアドレスの質問に進むようにしてください。ノードの追加はしないでください。
    ```

    > **注意:** **Copilotで編集する** パネルが表示されていない場合、もう一度トピックを作り直してください。

1. **更新**を選択します。

    ![質問ノード](../media/mailaddress-node.png)

    > **注意**: メッセージは前のノードの*Name*変数を含むように更新され、上のスクリーンショットのようになるはずです。Copilotで編集するが質問ノードを正しく更新しなかった場合は、元に戻すを選択し、別のプロンプトで再試行してください。
...
> **チェックポイント — 変数の確認**
>
> 「お客様詳細」トピックには、Copilot が自動生成した以下の**3つの役割**の変数が存在するはずです。変数名は生成のたびに異なる場合があります（例: `Name`、`EmailAddress`、`DetailsCorrect`）:
>
> | 役割 | 変数名の例 |
> |---|---|
> | 顧客の名前を保存する | `Name` |
> | メールアドレスを保存する | `EmailAddress` |
> | 詳細確認の「はい/いいえ」を保存する | `DetailsCorrect` |
>
> **変数名が例と異なる場合でも問題ありません。** Lab05・Lab07 でこれら3変数を使用するため、ご自身の環境の実際の変数名をメモしておいてください。リネームは不要です。
...
1. **質問を入力するか、必要なことを説明してください**テキストボックスに次のテキストを入力します:
...
1. **フレーズを追加**の下に次のテキストを入力し、**+** アイコンを選択します:
