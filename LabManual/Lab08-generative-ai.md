...
1. **エージェントの応答に生成AIオーケストレーションを使用しますか？**で**はい - 利用可能なツールとナレッジを適宜使用して、応答を動的にします**を選びます。
    ...
    > **注意:** Lab03〜07ではトピック制御の学習のため、生成AIオーケストレーションをオフにしていました。このラボからは生成AIがツールとナレッジを自律的に選択・使用する動作を学ぶため、オンに戻します。
...
1. **会話ブースティング**トピックを選択します。

1. キャンパス内の**生成応答を作成する**ノードを確認します。
...
1. **認証**を選択します。
    ...
    > **注意:** Lab05では「認証なし」を設定していましたが、このラボからはDataverseへのアクセスやユーザー情報の取り扱いのために、Microsoftアカウントでの認証が必要になります。
...
1. **+ ナレッジを追加**を選びます。
    ...
    1. **One Drive**を選び、**項目の参照**、**マイファイル（左側のファイル状のアイコン）**と進み、事前にコピーしたファイル **`SummitRealtyCaseStudy.docx`** を選んで **内容の確認** 、 **エージェントに追加する**を選択します。
...
1. **保存** します。
    ...
    ![ナレッジタブ（2件のナレッジソース一覧）のスクリーンショット](../media/Lab08_015.JPG)
...
1. **条件**ノードの下にある **+** アイコンを選択し、**詳細**を選択してから、**生成応答**を選びます。

    ![ノード追加メニュー（詳細＞生成応答）のスクリーンショット](../media/Lab08_016.JPG)
...
1. エージェントを探索して、ナレッジソースをどのように使用するかを確認しましょう。例えば:
  ...
  - `Summit RealtyはどのCRMを使用していますか？`
    ...
    ![エージェントのテスト画面（応答表示）のスクリーンショット](../media/Lab08_017.JPG)
