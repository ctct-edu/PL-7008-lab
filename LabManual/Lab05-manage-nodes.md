...
1. 3つのトピック変数の右側にあるチェックボックスを選択します（お客様詳細トピックで作成された3つの変数：顧客の名前・メールアドレス・詳細確認の回答を保存したものです）。

    > **注意:** 有効にする3変数（例: `Name`、`EmailAddress`、`DetailsCorrect`）は Lab04 でCopilotが自動生成したものです。それぞれ「顧客の名前」「メールアドレス」「情報確認の回答」を格納しており、この設定により他のトピックからもアクセス可能になります。変数名が異なる場合は、同じ役割の変数を選択してください。
...
1. **メッセージを入力する**フィールドに次のテキストを入力します:
    ...
    ![内見予約トピックのメッセージノード編集中のスクリーンショット](../media/Lab05_014.JPG)
...
1. **条件**ノード内の*変数を選択する*から**DetailsCorrect**変数を選択します（Lab04でCopilotが生成した「はい/いいえ」確認の回答を保存する変数。名前が異なる場合は同じ役割の変数を選択）。
...
1. **保存**を選択します。
    ...
    ![PropertyName変数のプロパティパネルのスクリーンショット](../media/Lab05_016.JPG)
...
1. 左側の**質問**ノードの下にある **+** アイコンを選択し、**メッセージを送信する**を選びます。
...
1. **どの物件を見学したいですか？** のプロンプトに次のテキストを入力します:
...
1. **認証**を選択します。
    ...
    > **注意:** ここでは動作確認のため「認証なし」を選択します。この設定では誰でもエージェントを利用できます。Lab08でDataverseの個人データにアクセスする段階で、Microsoft認証に切り替えます。
