...
1. 左下に表示されいている地球儀上のアイコンをクリックします。 **環境の切り替え** が呼び出せるため、 **現在の環境** が **copilotstudio** から始まる名称の環境に切り替わっていることを確認します。

1. 左側のナビゲーションで**エージェント**を選びます。

    ![エージェント一覧（空のエージェントを作成するボタン）のスクリーンショット](../media/Lab03_01.png)

1. 右上の**新しいエージェント** の右側にある下向きの矢印をクリックして、ドロップダウンリストから **標準オーケストレーションを使用して構築する** にあるエージェント（下側）を選択します。

    ![エージェント作成ダイアログのスクリーンショット](../media/Lab03_02.png)
