...
1. **エンティティ**タブを選びます。エージェント用の事前構築済みエンティティのリストが表示されるはずです。
    ...
    ![設定画面のエンティティタブのスクリーンショット](../media/lab06_01.png)

### タスク1.2 - 物件タイプエンティティを作成する

1. **+ エンティティを追加** を選択し、 **+ 新しいエンティティ** を選びます。

    > 画面表示のバグにより、**+ VariableTypesResources.AddNewEntity.Create** と表示されている場合があります。その場合は当該ボタンをクリックした後に下側の項目を選択、右側に表示されたペインを確認して閉じることで正しい項目に更新されます。
